...
- **认人不由边车做,而且假定画面里只有已登记成员**。边车自己不识别任何人:名字由
  miloco 侧的本地 ReID 比对身份库得出,以名册(`roster`)随请求送来,模型只负责把
  给定的名字贴到给定的 bbox 上。随之而来两条限制 ——(1)判成员的相似度阈值 0.70 是
  拿电视误检标定的,**从未拿人标定**:未登记的人对一个健康库 top1 中位 0.818,实测
  33 个陌生人框 33 个全被安上了成员名;(2)身份库过期不是"认不出"而是"高置信度
  认错"(36 天前的库逐人 0/14)。详见 miloco 侧 `perception/local_vision/identity.py`。
