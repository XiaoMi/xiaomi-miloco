...
7. **拾音关闭的相机听不到声音。** 住户在面板可按相机关闭拾音（`scope camera list` 的 `voice_in_use=false`）——该相机音频完全不被处理，`perceive query` 也听不到它的现场声音（问"有没有人说话"只能靠画面判断）；视频感知照常。属住户隐私设置，如实告知即可、不要试图绕过。
