...
7. **拾音关闭的相机听不到声音。** 拾音可按相机开关（面板或 `scope camera mic-on/mic-off`，见 miloco-miot-scope；状态看 `scope camera list` 的 `voice_in_use`）——`voice_in_use=false` 时该相机音频完全不被处理，`perceive query` 也听不到它的现场声音（问"有没有人说话"只能靠画面判断）；视频感知照常。查询时如实告知「该摄像头拾音已关闭」；除非用户明确要求打开，不要自行改动。
