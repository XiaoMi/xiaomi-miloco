...
- ❌ 不要在 tool 调用前 / 后发"收到视频,正在分析..."等中间状态消息 —— tool 调用几秒到几十秒的延迟可接受, 用户等就是了。本步末尾只发这两样:拼图(走 `message`)+ 一段文字(走 `content`),中途不发进度碎话
- 🚨 **发图必须用 `message` 工具、且只发图**: 飞书 DM `capabilities=none`, `MEDIA:` 内联标记不渲染, 用户看不到图。正确调用 `message action=send media=<拼图本地路径>`, **这条 message 的 `text` 留空、不写文字说明**;**任何子分支的发图都走这条路径**, 不在 reply 里嵌 `MEDIA:` / `![](path)` / `<img>`。
- 🚨 **候选说明/追问用普通文字回复(assistant `content`)单独发**: 发完拼图后, 必须再像平时聊天那样**直接回一句文字**(不经任何工具), 内容见各子分支的首轮文字模板。**别把文字塞进 `message` 的 `text`, 也别只发张图就收尾**。(即: 图走 `message`、文字走 `content`, 两者分开——与宠物注册一致。)
