...
- **摄像头拾音（音频是否参与感知）**：**默认关闭**——新装/新增相机静音起步，声音完全不被处理（不转写、不上云、听不到语音指令），视频照常感知。仅在**用户明确要求**时用 `scope camera mic-on <did>` 按机位开启；`mic-off` 关回默认。从属于视频感知：感知已 disable 的相机不能设拾音。
- 拾音开关的定位是「**每摄像头的信噪比开关**」，且取保守面：**默认全关**，只在用户明确要求时开某台（安静房间、确有语音交互需求的机位）；嘈杂位（对着电视 / 街边窗口）保持关闭。用户抱怨某摄像头声音类**误报**时，确认其拾音状态、关回默认即是治理手段。
...
- **摄像头 `mic-on/mic-off <did>...`**：拾音批量开/关，同款批量 did 语义。**拾音默认关闭**，`mic-on` 仅在用户明确要求时用；`mic-off` = 回到默认（该相机声音完全不被处理）。仅感知已启用(in_use=true)的相机可设，感知已关闭时整批被拒。改动即时生效、无需重启。
