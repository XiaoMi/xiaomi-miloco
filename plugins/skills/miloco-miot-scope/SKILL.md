...
- `list` 输出每项含 `in_use`（是否启用）；camera 额外带 `room_name`（房间名——相机 `name` 常是「小米智能摄像机 2 代」这类泛称，按房间定位机位靠它）、`is_online`（设备在线，= `cloud_online` AND `lan_reachable`，这两个分量也一并透出）、`connected`（流已订阅）、`awake`（该路镜头开关，`null`=机型无此属性或读取失败）、`voice_in_use`（声音开关）、`crop_in_use`（该路智能裁切**存储偏好**，默认 true）、`crop_effective`（智能裁切**生效态** = `in_use` AND 全局双闸 AND `crop_in_use`）和 `perception_prompt`（该机位自定义感知须知）。`in_use`/`is_online`/`connected` 三者都 true = 正常采集，任一 false 即某层未就位。`voice_in_use=false` = 该相机声音完全不被处理（不转写、不上云、听不到指令），视频照常感知。
...
> **响应形状（下面各例共用）**：所有返回**相机列表**的命令（`list` / `enable` / `disable` /
> `mic-*` / `crop-*` / `prompt-*`）返回的行是同一个形状，即下面 `scope camera list` 那一份。
> 差别只有一处：**只有 `list`** 会过 CLI 展示层变换（多通道相机的 `did` 拼成 `did:chN`，并去掉
> `channel` / `channel_count`）；其余命令直接打印后端响应，`did` 是**裸物理 did** 且带
> `channel` / `channel_count`。为省篇幅，`list` 以外的示例只列出与该次操作相关的键，
> 其余用 `…` 略过——**不是**这些命令的响应真的更短。
...
$ miloco-cli scope camera list
  → {"code":0,"message":"ok","data":[
       {"did":"1154253569","name":"小米智能摄像机C700","room_name":"客厅",
        "cloud_online":true,"lan_reachable":true,"is_online":true,"voice_in_use":true,
        "awake":true,"in_use":true,"connected":true,"perception_prompt":"",
        "crop_in_use":true,"crop_effective":true},
       {"did":"1154253570:ch0","name":"小米智能摄像机C700","room_name":"次卧",
        "cloud_online":true,"lan_reachable":true,"is_online":true,"voice_in_use":false,
        "awake":true,"in_use":true,"connected":true,"perception_prompt":"",
        "crop_in_use":true,"crop_effective":true},
       {"did":"1154253570:ch1","name":"小米智能摄像机C700","room_name":"次卧",
        "cloud_online":true,"lan_reachable":true,"is_online":true,"voice_in_use":false,
        "awake":true,"in_use":true,"connected":true,"perception_prompt":"",
        "crop_in_use":true,"crop_effective":true},
       {"did":"1154253571","name":"小米智能摄像机C700","room_name":"门口",
        "cloud_online":true,"lan_reachable":true,"is_online":true,"voice_in_use":false,
        "awake":true,"in_use":true,"connected":true,"perception_prompt":"",
        "crop_in_use":true,"crop_effective":true}]}
#  ↑ 双摄（1154253570）在 list 里是**两行**、did 已拼成 `did:chN`（`channel` /
#    `channel_count` 已并入 did，list 输出里没有这两个键）；单摄保持裸 did。
#    这里的 did 可**直接复制**给 crop-off / prompt-set 精确到某一路。
...
$ miloco-cli scope camera disable 1154253569
  → {"code":0,"message":"ok","data":[
       {"did":"1154253569","name":"小米智能摄像机C700","channel":0,"in_use":false,"connected":false,…}]}

# 恢复被停用的摄像头
$ miloco-cli scope camera enable 1154253569
  → {"code":0,"message":"ok","data":[
       {"did":"1154253569","name":"小米智能摄像机C700","channel":0,"in_use":true,"connected":true,…}]}
...
$ miloco-cli scope camera mic-off 1154253569
  → {"code":0,"message":"ok","data":[
       {"did":"1154253569","name":"小米智能摄像机C700","channel":0,"voice_in_use":false,…}]}

# 「次卧很安静，把声音打开」——开声音
$ miloco-cli scope camera mic-on 1154253570
  → {"code":0,"message":"ok","data":[
       {"did":"1154253570","name":"小米智能摄像机C700","channel":0,"voice_in_use":true,…},
       {"did":"1154253570","name":"小米智能摄像机C700","channel":1,"voice_in_use":true,…}]}
#  拾音按**整台**存，双摄两路的 voice_in_use 一起变（与逐路的 crop / prompt 刻意不同）。

# 「次卧这台别裁了 / 只看整个画面」——画面**构图**类诉求，逐机位关裁切
#  注意与下面的「误识」场景分界：认错东西（把 A 当 B）→ 写感知须知；
#  构图粗细（裁主体 vs 送整幅）→ crop-off/crop-on。别互相套用。
$ miloco-cli scope camera crop-off 1154253570:ch0    # 双摄，只关 ch0 这一路（精确到 :chN）
  → {"code":0,"message":"ok","data":[
       {"did":"1154253570","name":"小米智能摄像机C700","channel":0,"crop_in_use":false,"crop_effective":false,…},
       {"did":"1154253570","name":"小米智能摄像机C700","channel":1,"crop_in_use":true,"crop_effective":true,…}]}
#  返回按**物理 did** 过滤：双摄两路都会回来（不只被点名的 ch0），只有 ch0 那行的值变了——
#  看到另一行仍是 true 是**正常的**，别当成没生效去重试。
#  关掉后：crop_in_use=false（存储偏好），crop_effective 随之 false

$ miloco-cli scope camera crop-on  1154253570        # 裸 did = 该台全部通道，开回默认
  → {"code":0,"message":"ok","data":[
       {"did":"1154253570","name":"小米智能摄像机C700","channel":0,"crop_in_use":true,"crop_effective":true,…},
       {"did":"1154253570","name":"小米智能摄像机C700","channel":1,"crop_in_use":true,"crop_effective":true,…}]}
...
$ miloco-cli scope camera prompt-set 1154253571 "本摄像头装在入户门内，画面右侧公共走廊里可见电梯门。电梯门开合与本户无关，不要据此判断有人回家/开门；只有画面正中的木色入户门开合才是本户事件。"
  → {"code":0,"message":"ok","data":[
       {"did":"1154253571","name":"小米智能摄像机C700","channel":0,"perception_prompt":"本摄像头装在入户门内……",…}]}

# 清除某台的感知须知（回到无自定义）
$ miloco-cli scope camera prompt-clear 1154253571
  → {"code":0,"message":"ok","data":[
       {"did":"1154253571","name":"小米智能摄像机C700","channel":0,"perception_prompt":"",…}]}
```
