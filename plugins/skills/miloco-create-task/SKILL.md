...
**绝不创建不带家庭时区的定时任务。** openclaw 对不带时区的 cron 按**宿主机时区**求值，宿主机时区 ≠ 家庭时区时整点全偏（曾把本该 21:00 的提醒错点）。家庭时区（IANA 名，如 `Asia/Shanghai`）取自注入的「## 时间与时区」块，下方 `<家庭时区>` 一律替换成该真实 IANA 名。

- **cron / every**：按 openclaw 语法在表达式末尾后缀 ` @ <家庭时区>`，如 `0 21 * * * @ Asia/Shanghai`。
- **at**：用 `miloco-cli time-compute` 产出的 ISO 已带时区偏移（如 `+08:00`），本身无歧义，直接传即可，无需再加后缀。
...
2. **Record=Y(progress) 且 target > 0** → 多时点提醒：
   ...
   - 装配：N 个时点合并装单 cron `0 H1,H2,...,Hn * * * @ <家庭时区>`，单 jobId 单 task_link（时区后缀必带，见 §Schedule.时区）
3. **其他**（无 target / Record=event / Record=duration / 无 Record） → 每周期单时点：day → `0 9 * * * @ <家庭时区>`；week → `0 9 * * 1 @ <家庭时区>`；month → `0 9 1 * * @ <家庭时区>`
...
# 调 OpenClaw cron 建提醒 cron（喝水无强场景 → 均分 10-20，N=min(8,6)=6 时点）：name="[drink_8_today] 喝水提醒"，cron="0 10,12,14,16,18,20 * * * @ <家庭时区>"（时区后缀必带，见 §Schedule.时区），message="调 miloco-cli task record get drink_8_today，按 derived.remaining 决定是否催"
...
# 调 OpenClaw cron 建提醒 cron（喝水无强场景 → 均分 10-20，N=min(8,6)=6 时点）：name="[drink_8_daily] 喝水提醒"，cron="0 10,12,14,16,18,20 * * * @ <家庭时区>"（时区后缀必带，见 §Schedule.时区），message="调 miloco-cli task record get drink_8_daily，按 derived.remaining 决定是否催"
