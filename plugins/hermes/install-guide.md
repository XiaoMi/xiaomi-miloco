...
description: Miloco Hermes Agent 插件安装引导，当用户说 "安装 miloco hermes" 时激活。
metadata:
  author: Miloco Team
  version: "1.0"
  date: 2026-06-21
...
# Miloco Hermes Agent 插件安装指南

## 概述

本 skill 指导 agent 通过 3 个阶段完成 Miloco 在 Hermes Agent 上的安装。脚本以 `agent` 模式运行，专为非交互终端设计，通过 JSON 输出与 agent 通信。
...
```bash
curl -LsSf https://github.com/XiaoMi/xiaomi-miloco/releases/latest/download/install.sh | bash -s -- --agent-prepare --agent hermes
```
...
**目标：** 重启服务、写入账号/模型配置、下载感知模型、安装 Hermes 插件。
...
| 参数 | 必需 | 说明 |
|------|------|------|
| `--agent hermes` | 是 | 安装 Hermes Agent 插件（含技能同步） |
...
> 安装完成！执行 `hermes gateway restart` 重启网关即可使用。
>
> 常用命令：
...
| hermes 未安装 | 参考 https://hermes-agent.nousresearch.com 安装 |
