 ...
 *   1. 默认值必须与后端 settings.yaml 一致：rule_only_input=image、last_frame_only=false
 *      （图片输入 + 多帧）。默认值写反了，新装机的行为就跟 UI 上显示的对不上。
 *   2. payload 的字段名必须是后端 PerceptionConfigBody 认的
 *      perception_input / image_last_frame_only（写错会被 pydantic 静默忽略，
 *      保存看起来成功、实际没生效）。
 *   3. 恢复默认要回到"图片 + 多帧"，而不是把开关带到某个中间态。
 *
...
describe("buildPerceptionInputPayload", () => {
  ...
  it("恢复默认 = 图片 + 多帧", () => {
    ...
    ).toEqual({ perception_input: "image", image_last_frame_only: false });
  });
});
