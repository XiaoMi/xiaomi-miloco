 ...
 *
 * 为什么要抽出来：明细表**同一行**上挂着两个 fixed 浮层——模型列那格的 URL 气泡，
 * 与末尾操作格的清理菜单。各写一份落点算法的结果是，一个会在视口底部翻转、另一个
 * 不会：明细末几行常常正好贴着视口底，不翻转的那个整体落到视口外，而滚动会关掉浮层
 * （fixed 会失锚），于是住户连滚下去看一眼都做不到，观感就是「这个按钮坏了」。
 * 同一份认知只落实一半，是因为它散在两个组件里；收成一个函数后，改一处就是改所有处。
 */
