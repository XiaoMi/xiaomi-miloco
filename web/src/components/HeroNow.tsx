...
function CameraScheduleDialog({
  ...
  const submit = async () => {
    ...
    // 只有「每天」生效时一天才是环形的；星期受限时跨午夜段的次日那半属于另一天，
    // 环形缝合会静默把覆盖挪一天（见 cameraSchedule.ts 的 wrapAcrossDays 注释）。
    const wrapAcrossDays = weekdays.length === CAMERA_SCHEDULE_WEEKDAYS.length;
    // 合并与开关态无关：后端无论 enabled 真假都校验 windows（零长/重叠都会 400），
    // 关掉定时时也要先归一，否则用户只是想关开关却吃一条后端校验报错。
    const mergedWindows = mergeScheduleWindows(normalizedWindows, {
      wrapAcrossDays,
    });
    ...
    if (!wrapAcrossDays && hasCircularOverlap(mergedWindows)) {
      setError(t("hero.scheduleCircularOverlap"));
      return;
    }
    ...
    try {
      ...
      // 上层已 toast 并 rethrow（App.tsx），这里接住避免 unhandled rejection；
      // 同时把原因留在对话框内——弹窗不关，用户能对着报错改时段。
      setError(e instanceof Error ? e.message : t("hero.scheduleInvalid"));
    }
  };
