...
export function UsageClearDialog({
  ...
  scope: ClearScope;
  /**
   * 日聚合表里已有的最早 / 最新日期（YYYY-MM-DD），null = 表为空或接口未给。
    ...
    >
      ...
      >
        ...
        {/* 日聚合表只有天粒度：跨天范围会连带删掉边界当天更早的记录。这是日聚合的
            ...
            但只在边界那天**落在日表已有的日期区间里**时才说。两头各挡一类落空：滚存
            截止天对齐、只搬更早的行，所以「近 24 小时」那种边界日不在表里（上界）；盒子
            运行天数短于所选范围时，边界日早于表里最早那天，删掉的都是本就在范围内的整天
