/**
 * 「模型」tab 内联的精简性能视图:只含 工具条 + KPI 卡 + 处理耗时 + Gate 过滤率。
 *
 * 完整性能页仍在 #perf(PerfPage)保留全部区块;这里只复用前几个块,
 * 且只拉这几块需要的接口(summary / latency / gate),不发起其余 perf 请求。
 *
 * 「紧凑显示」把处理耗时与 Gate 过滤率并排放一行——两张图都是同一时间窗、同一分桶的
 * 折线,并排比上下叠更容易对着看「这一段过滤率高的时候耗时是不是也涨了」。仅在够宽
...
export function PerfInline() {
  ...
  const latency = useAsync(() => getPerfLatencySeries(windowKey, bucket), [windowKey, bucket], {
  ...
  // 收起时留一个最要紧的读数。选 omni 耗时 P95：它是「感知这一段跟不跟得上」的直接指标，
  // 且与 KPI 卡里同名那格用同一个字段与同一条越界判据（后端给的 p95_behind_omni），
  // 不另立标准。
  //
  // 上色口径对齐两处既有约定：常态整串 text-text-secondary，与 Token 用量的收起摘要一致；
  // 越界时**只有数值**变色、且用 text-error，与 KPI 卡一致——整串染色会让这行摘要在
  // 一排卡头里过分抢眼，而它只是个参考值。
  const omniP95 = summary.data?.p95_ms_omni;
  const behind = summary.data?.p95_behind_omni ?? false;
  const collapsedSummary =
    omniP95 == null ? undefined : (
      <span className="text-text-secondary">
        {t("perf.kpiOmniLatencyP95")}{" "}
        <span
          className={`num ${behind ? "text-error font-semibold" : ""}`}
        >
          {Math.round(omniP95)} ms
        </span>
      </span>
    );
  ...
  useEffect(() => {
    if (summary.data || latency.data || gate.data) setUpdatedAt(new Date());
  }, [summary.data, latency.data, gate.data]);
  ...
  return (
    ...
    >
      ...
      >
        <PerfLatencyChart state={latency} bucket={bucket} windowMs={windowMs} embedded />
        <div
          className={
            compact
