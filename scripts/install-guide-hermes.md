...
> **关键纪律：必须按 2.1 → 2.2 → 2.3 → 2.4 顺序逐项询问。每项完成（用户回复"好了"/"ok"/"继续"）才发下一项。禁止一次性贴出全部步骤。**

#### 2.0 绑定通知渠道

**目标：** 让 miloco 知道通知推送到哪里。默认用当前对话所在的 IM 平台，用户以后可以换。

agent 直接调 miloco_notify_bind（无需问用户）：
```
miloco_notify_bind action=switch target=<当前平台名>
```
平台名从当前对话上下文取：飞书→feishu，微信→weixin，Telegram→telegram。

然后发：
> 通知消息默认推送到 **{当前平台}**。以后想换就在任意 IM 说"通知发到微信"。
