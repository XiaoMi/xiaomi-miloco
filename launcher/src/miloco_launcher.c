...
// Environment is inherited (the LaunchAgent plist supplies MILOCO_HOME /
// PATH / HOME / TZ etc.). Note it deliberately does NOT set MILOCO_SUPERVISED:
// that flag means "an external supervisor collects and rotates my stdout",
// which launchd does not do — setting it makes the backend skip its own log
// rotation and grow unbounded. See _ENV_PASSTHROUGH_DENY in
// cli/src/miloco_cli/commands/service.py before adding it here.
//
// Build (on a Mac, once per launcher change; the signed .app is then vendored
// into the repo). MUST be a universal binary — the one vendored bundle ships to
// both arm64 and x86_64 Macs, and a single-arch build fails to exec on the other
// (launchd then cannot start the backend at all). Note both -arch flags go in
// ONE clang invocation; two separate commands writing the same output path would
// leave only whichever ran last:
//   clang -arch arm64 -arch x86_64 -O2 -o Contents/MacOS/miloco miloco_launcher.c
//   lipo -archs Contents/MacOS/miloco      # expect: x86_64 arm64
//   codesign -s - --force --identifier com.xiaomi.miloco.backend <bundle>.app
//   codesign -v <bundle>.app
//
// CI enforces part of the above (.ci/launcher-guard.py): it hard-fails if the
// vendored binary is no longer a fat arm64+x86_64 bundle or if the Info.plist
// identity keys changed, and warns when launcher/src/ changes without
// re-vendoring launcher/darwin/.
