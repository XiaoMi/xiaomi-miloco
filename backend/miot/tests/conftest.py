...
TEST_BAIDU_OAUTH2_CLIENT_ID: str = (
    environ.get("TEST_BAIDU_OAUTH2_CLIENT_ID") or "<Your Client Id>"
)
TEST_BAIDU_OAUTH2_CLIENT_SECRET: str = (
    environ.get("TEST_BAIDU_OAUTH2_CLIENT_SECRET") or "<Your Client Secret>"
)
TEST_BAIDU_OAUTH2_REDIRECT_URI: str = (
    f"https://{PROJECT_CODE}.api.mijia.tech/login_redirect"
)
...
def pytest_configure(config):
    ...
    为什么不反过来（给联调脚本打 ``integration``、CI 跑 ``-m "not integration"``）：
    那样「新增单测默认进门禁」确实更省心,但 ``test_client.py`` / ``test_cloud.py``
    是混合文件——各有一批**未打标**的 OAuth/云端联调用例与纯 mock 用例同住,排除式
    会把它们一起拖进门禁,门禁必红;要落反向方案得先把这些用例逐条打上
    ``integration``,那是另一件事。在此之前,**新加纯 mock 单测请顺手补一行**
    ``pytestmark = pytest.mark.unit``（或用例级 ``@pytest.mark.unit``）,否则它不进
    门禁、白写。
    """
