...
class MIoTCert:
    ...
    async def verify_ca_cert_async(self) -> bool:
        ...
        A hash mismatch means the on-disk copy is stale or corrupt (truncated
        write, restored backup, rotated CA). The bundled constant is the source
        of truth, so rewrite from it once and re-verify instead of failing
        permanently — otherwise local control stays dead until someone deletes
        the file by hand.
        """
        for attempt in (0, 1):
            ...
            if ca_data is not None:
                hash_str = hashlib.sha256(ca_data).hexdigest()
                if hash_str == MIHOME_CA_CERT_SHA256:
                    return True
                if attempt == 1:
                    _LOGGER.error(
                        "CA cert hash mismatch after rewrite "
                        "(expected %s); remove %s manually and restart",
                        MIHOME_CA_CERT_SHA256,
                        self.ca_file,
                    )
                    return False
                _LOGGER.warning(
                    "CA cert hash mismatch (%s); rewriting from bundled CA",
                    hash_str,
                )
                await self.remove_ca_cert_async()
