...
class _MIoTLanDevice:
    ...
    def keep_alive(self, ip: str, if_name: str) -> None:
        ...
        if self._if_name != if_name:
            ...
            # 同网段目标也纳入单播探测后，一台设备每轮会同时回广播（真实网卡名）
            # 与单播（"unicast"）两个包，该字段每轮翻转两次。它只作诊断、不参与
            # 发包选路，降到 debug 避免把 INFO 级的上下线事件刷没。
            _LOGGER.debug(
                "device if_name change, %s, %s", self.did, self._if_name
            )
...
class MIoTLan:
    ...
    def __create_unicast_socket(self) -> None:
        ...
        try:
            ...
            # add_reader 失败时 sock 已建成但没人持有它：显式关掉，否则按需补建
            # 每轮重试（最小扫描周期 5s）都会泄漏一个 fd，fd 耗尽场景只会恶化。
            if sock is not None:
                sock.close()
