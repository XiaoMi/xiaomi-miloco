...
def test_voice_enabled_camera_dids_default_empty():
    """默认关闭：白名单缺省为空 = 没有任何相机开启拾音。"""
    kv = _FakeKV()
    assert miot_filter.voice_enabled_camera_dids(kv) == set()


def test_voice_enabled_camera_dids_with_values():
    kv = _FakeKV(
        {ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY: json.dumps(["c1", "c2"])}
    )
    assert miot_filter.voice_enabled_camera_dids(kv) == {"c1", "c2"}


def test_voice_whitelist_is_orthogonal_to_feed_deny():
    """拾音白名单与感知黑名单互不影响：改一个不动另一个。"""
    ...
    assert miot_filter.voice_enabled_camera_dids(kv) == {"c2"}
...
def test_set_camera_voice_in_use_whitelist_storage():
    """对外语义不变（in_use=True 开启），底层写白名单（默认关闭）。"""
    kv = _FakeKV()
    # voice_in_use=True adds to whitelist
    assert miot_filter.set_camera_voice_in_use(kv, "c1", True) == (["c1"], True)
    assert miot_filter.set_cameras_voice_in_use(kv, ["c2", "c3"], True) == (
        ["c1", "c2", "c3"],
        True,
    )
    # voice_in_use=False removes from whitelist（回到默认关闭）
    assert miot_filter.set_camera_voice_in_use(kv, "c1", False) == (["c2", "c3"], True)
    # no-op re-toggle（关一个本就不在白名单的）→ changed=False
...
async def test_list_cameras_with_state_voice_flags():
    """voice_in_use 是存储偏好：在拾音白名单即 True（**默认 False**），与 in_use 正交。"""
    cameras = {"c1": _camera("c1", home_id="H1"), "c2": _camera("c2", home_id="H1")}
    kv = _FakeKV({
        ScopeConfigKeys.HOME_WHITE_LIST_KEY: json.dumps(["H1"]),
        ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY: json.dumps(["c1"]),
    })
    svc = _make_service(devices=dict(cameras), cameras=cameras, kv=kv)
    out = await svc.list_cameras_with_state()
    by_did = {c["did"]: c for c in out}
    assert by_did["c1"]["voice_in_use"] is True    # 在拾音白名单
    assert by_did["c1"]["in_use"] is True          # 感知启用（正交）
    assert by_did["c2"]["voice_in_use"] is False   # 默认关闭（未显式开启）


@pytest.mark.asyncio
async def test_toggle_camera_voice_writes_whitelist():
    kv = _FakeKV({ScopeConfigKeys.HOME_WHITE_LIST_KEY: json.dumps(["H1"])})
    svc = _make_service(
        devices={"c1": _camera("c1")}, cameras={"c1": _camera("c1")}, kv=kv
    )
    ...
    assert json.loads(kv.get(ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY)) == ["c1"]

    res = await svc.toggle_camera_voice([{"did": "c1", "voice_in_use": False}])
    assert any(c["did"] == "c1" and c["voice_in_use"] is False for c in res)
    assert json.loads(kv.get(ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY)) == []
...
async def test_toggle_camera_voice_rejected_when_camera_disabled():
    ...
    # 拒绝后不落库
    assert kv.get(ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY) is None


@pytest.mark.asyncio
async def test_toggle_camera_off_on_preserves_voice_preference():
    """关相机不改写拾音白名单：off→on 循环后拾音偏好原样保留（存储偏好不落库为「自动关」）。"""
    kv = _FakeKV({ScopeConfigKeys.HOME_WHITE_LIST_KEY: json.dumps(["H1"])})
    svc = _make_service(
        devices={"c1": _camera("c1")}, cameras={"c1": _camera("c1")}, kv=kv
    )
    ...
    assert json.loads(kv.get(ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY)) == ["c1"]
    # 重新开相机感知 → 旧拾音偏好仍在（voice_in_use=True）
    await svc.toggle_camera([{"did": "c1", "in_use": True}])
    assert json.loads(kv.get(ScopeConfigKeys.CAMERA_VOICE_WHITE_LIST_KEY)) == ["c1"]
    out = await svc.list_cameras_with_state()
    by_did = {c["did"]: c for c in out}
    assert by_did["c1"]["in_use"] is True
    assert by_did["c1"]["voice_in_use"] is True  # 偏好保留（显式开启不因关相机被抹）
