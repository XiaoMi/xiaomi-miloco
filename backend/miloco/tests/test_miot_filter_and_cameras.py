...
def test_as_day_intervals_until_midnight_skips_zero_slice():
    assert miot_filter._as_day_intervals(22 * 60, 0) == [(22 * 60, 24 * 60)]
    assert miot_filter._as_day_intervals(22 * 60, 7 * 60) == [
        (22 * 60, 24 * 60),
        (0, 7 * 60),
    ]


def test_camera_schedule_until_midnight_covers_2359():
    """「22:00-00:00」在 23:59 仍应感知，不能被截成 23:59 留下 60 秒盲区。"""
    schedule = miot_filter.normalize_camera_schedule({
        "enabled": True,
        "windows": [{"start": "22:00", "end": "00:00"}],
    })
    tz = ZoneInfo("Asia/Shanghai")
    assert schedule["windows"] == [{"start": "22:00", "end": "00:00"}]
    assert miot_filter.camera_schedule_paused(
        schedule,
        datetime(2026, 6, 21, 23, 59, tzinfo=tz),
    ) is False
    assert miot_filter.camera_schedule_paused(
        schedule,
        datetime(2026, 6, 21, 0, 0, tzinfo=tz),
    ) is True
...
@pytest.mark.asyncio
async def test_disable_schedule_bare_payload_keeps_weekdays():
    """裸 PUT {"enabled": false} 不得把 weekdays 重置成每天（PR body 承诺关闭时保留）。"""
    kv = _FakeKV({ScopeConfigKeys.HOME_WHITE_LIST_KEY: json.dumps(["H1"])})
    svc = _make_service(
    ...
    res = await svc.set_camera_schedule("c1", CameraSchedule(enabled=False))

    assert res["schedule"]["enabled"] is False
    assert res["schedule"]["weekdays"] == [0, 2]
    assert res["schedule"]["windows"] == [{"start": "08:00", "end": "20:00"}]
