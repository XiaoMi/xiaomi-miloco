...
def client(tmp_path, monkeypatch):
    ...
    (tmp_path / "config.json").write_text(
        _json.dumps({"model": {"omni": {"label": "", "model": "xiaomi/mimo-v2.5",
...
def test_put_activate_true_success_requires_probe_ok(client, mock_probe):
    ...
    r = client.put("/api/admin/omni-config",
                   json={"label": "甲", "model": "m1", "base_url": "https://x/v1",
                         "api_key": "sk-xxxxxx", "activate": True})
...
def test_put_activate_true_rejects_unreachable(client, mock_probe):
    ...
    r = client.put("/api/admin/omni-config",
                   json={"label": "甲", "model": "m1", "base_url": "https://nope/v1",
                         "api_key": "sk-x", "activate": True})
    assert r.status_code == 400
...
def test_put_activate_false_skips_preflight(client, mock_probe):
    ...
    r = client.put("/api/admin/omni-config",
                   json={"label": "备用", "model": "m1", "base_url": "https://x/v1",
                         "api_key": "sk-x", "activate": False})
    assert r.status_code == 200
    data = r.json()["data"]
...
def test_activate_success_when_probe_ok(client, mock_probe):
    client.put("/api/admin/omni-config",
               json={"label": "甲", "model": "m1", "base_url": "https://x/v1",
                     "api_key": "sk-xxxxxx", "activate": False})
