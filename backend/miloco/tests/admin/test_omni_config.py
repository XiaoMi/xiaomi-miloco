...
def test_put_preflight_failure_on_unchanged_active_profile_updates_record(
    ...
    写进记录,不能让界面停在旧的绿 ●。"""
    # 指纹只在服务端用于判断记录是否作废,响应刻意不下发(见 test_response_never_exposes_fingerprint),
    # 故要拿它比对时直读 kv 里的原始记录,而非从接口返回里取。
    ...
    before = next(p for p in _get(client)["profiles"] if p["label"] == "甲")
    assert before["last_verified"]["ok"] is True
    fp_before = json.loads(
        manager.kv_repo.get(OmniConfigKeys.LAST_VERIFIED_KEY) or "{}"
    )["甲"]["fingerprint"]
    ...
    # 一字未改:写下的仍是同一份三元组的指纹
    assert (
        json.loads(manager.kv_repo.get(OmniConfigKeys.LAST_VERIFIED_KEY) or "{}")["甲"][
            "fingerprint"
        ]
        == fp_before
    )
...
def test_response_never_exposes_fingerprint(client):
    """契约:last_verified 里绝不出现 fingerprint —— 它是 api_key 派生值,下发等于给客户端
    一个可离线校验 api_key 猜测的 oracle。它必须仍存在 kv 里(作废判断靠它),只是不许
    出现在任何接口响应中。"""
    import re
    ...
    client.put("/api/admin/omni-config", json=body)

    # 前提:kv 里确实存了指纹 —— 否则本用例守不住任何东西(把指纹整个删掉也能"通过")
    raw = json.loads(manager.kv_repo.get(OmniConfigKeys.LAST_VERIFIED_KEY) or "{}")
    assert re.fullmatch(r"[0-9a-f]{16}", raw["甲"]["fingerprint"])

    def _assert_clean(payload: object) -> None:
        assert "fingerprint" not in json.dumps(payload, ensure_ascii=False)

    _assert_clean(_get(client))
    _assert_clean(client.put("/api/admin/omni-config", json=body).json())
    _assert_clean(client.post("/api/admin/omni-config/test", json=body).json())
...
def test_fingerprint_is_salted_by_device_uuid(client):
    """摘要必须由 kv 里的设备 UUID(每安装一次的随机盐)派生:换了盐,同一份
    model/base_url/api_key 算出的摘要必须不同;且两个摘要都不等于无盐的裸 sha256 ——
    否则拿到摘要就能离线校验 api_key 猜测,与掩码打码的意图相抵。"""
    import hashlib

    from miloco.admin.router import _omni_fingerprint, manager
    from miloco.database.kv_repo import SystemConfigKeys

    model, base_url, api_key = "m-salted", "https://x/v1", "sk-salt-me-please"
    unsalted = hashlib.sha256(
        f"{model}\x00{base_url}\x00{api_key}".encode()
    ).hexdigest()[:16]

    manager.kv_repo.set(SystemConfigKeys.DEVICE_UUID_KEY, "uuid-aaaa")
    fp_a = _omni_fingerprint(model, base_url, api_key)

    manager.kv_repo.set(SystemConfigKeys.DEVICE_UUID_KEY, "uuid-bbbb")
    fp_b = _omni_fingerprint(model, base_url, api_key)

    assert fp_a != fp_b
    assert fp_a != unsalted
    assert fp_b != unsalted


def test_fingerprint_is_not_a_bare_fast_hash(client):
    """钉住改法:指纹必须走带工作因子的 KDF,不能再是「三元组直接喂一次快哈希」——后者
    正是 CodeQL py/weak-sensitive-data-hashing 判为「口令类数据走了非慢哈希」的模式。同时
    钉住轮数:改回 1 就等于退回单次 HMAC(已实测二者数学等价),该模式的实质没变。"""
    import hashlib
    import hmac

    from miloco.admin.router import (
        _FINGERPRINT_PBKDF2_ROUNDS,
        _omni_fingerprint,
        manager,
    )
    from miloco.database.kv_repo import SystemConfigKeys

    model, base_url, api_key = "m-kdf", "https://x/v1", "sk-not-a-fast-hash"
    triple = f"{model}\x00{base_url}\x00{api_key}".encode()
    salt = b"uuid-kdf"

    manager.kv_repo.set(SystemConfigKeys.DEVICE_UUID_KEY, "uuid-kdf")
    fp = _omni_fingerprint(model, base_url, api_key)

    # 旧写法(告警模式):一次快哈希,盐做 key 或做 msg 都不允许
    assert fp != hmac.new(salt, triple, hashlib.sha256).hexdigest()[:16]
    assert fp != hashlib.sha256(triple + salt).hexdigest()[:16]
    # 工作因子必须 >1(rounds=1 等价于单次 HMAC),且落在交互延迟预算内(见常量处实测)
    assert 1 < _FINGERPRINT_PBKDF2_ROUNDS <= 10_000
    # 就是 PBKDF2-HMAC-SHA256 在该工作因子下的前 16 位
    assert (
        fp
        == hashlib.pbkdf2_hmac(
            "sha256", triple, salt, _FINGERPRINT_PBKDF2_ROUNDS
        ).hex()[:16]
    )
