...
async def test_audio_frame_callback_snapshots_connections(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            miot_service=SimpleNamespace(
                start_audio_stream=AsyncMock(),
                stop_audio_stream=AsyncMock(),
                get_audio_codec=lambda did, ch: "opus",
            )
        ),
    )
    mgr = ws_mod.MIoTAudioStreamManager()
...
async def test_audio_log_records_render_without_format_error(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO):
        cid = await mgr.new_connection(_FakeWS(), "user", "tok", "cam1", 0)
        await mgr.close_connection("user", "tok", "cam1", 0, cid)

    rendered = [record.getMessage() for record in caplog.records]
    assert any("Start audio stream" in msg for msg in rendered)
    assert any("No connection, stop audio stream" in msg for msg in rendered)


async def test_audio_frame_callback_stops_stream_when_no_subscriber(monkeypatch):
    """自愈路径：帧到达但连接表已空 → 在 per-camera 锁内复核后 stop。

    这是唯一在锁外的 stop 路径（帧回调刻意不整体拿锁——每帧等起停流的 await
    不划算），但「查表 → await stop」必须原子：check 与 stop 之间 new_connection
    可能刚开好流并放入订阅方，锁外 stop 会把新流杀掉 ⇒ 订阅方永久静音（帧不再
    来、回调不再触发，无看门狗兜底）。锁内复核后仍为空才 stop；拿锁无死锁——
    new/close 持锁只等自己的 await，从不等回调。
    """
    stop = AsyncMock()
    monkeypatch.setattr(
        ws_mod,
    ...
    callback = mgr._MIoTAudioStreamManager__audio_stream_callback
    await callback("cam1", b"\x00", 0, 0, 0)
