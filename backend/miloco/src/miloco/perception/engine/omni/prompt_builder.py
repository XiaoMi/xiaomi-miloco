...
def build_query_prompt(
    ...
    if resolved_visual_mode == "video":
        # query 不接 crop(v1 范围外);兜掉历史配置里可能残留的 0，避免编码 scale=0。
...
def build_fused_payload(
    ...
    if resolved_visual_mode == "frames":
    ...
    adaptive = (
        _maybe_encode_adaptive(packets, region_ok=_candidate_bbox_ok)
        if resolved_visual_mode == "video"
        else None
    )
    ...
    if resolved_visual_mode == "video" and video_b64 is None:
        video_b64, media_info = _encode_batch_video(
...
def _build_fused_user_content(
    *,
    ...
    # 5. 主视觉输入：图片模型收到固定关键帧序列，视频模型收到 MP4。
