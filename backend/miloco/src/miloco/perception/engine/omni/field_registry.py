...
class FieldSpec:
    ...
    spec_md_audio —— audio-only 路由专用「字段说明」变体（纯音频无画面，视觉语言全部剔除）；
        为 None 时 audio 路由沿用 video 版——**仅当 spec_md 完全视觉无关时才允许留 None**，
        spec_md 一旦引入画面措辞（如画面互证）必须同步提供本变体，否则视觉语言泄入纯音频
        prompt（无画面时「画面看不到迹象」恒空真，会误抑制真实安全事件声）。
    """
...
- 报事件声必须有**具体声学证据**：听感上能说清那是什么样的声音（质地/次数/持续，如「两声清脆的碎裂声」「连续急促的哭声」）才算听到了；只是「有点像 / 大概是」某类事件 → 证据不足，不报
- 有画面时与画面互证：玻璃破碎、婴儿哭、警报这类重大事件声，若画面视野内看不到任何对应迹象（无碎片、无婴儿、画面空无一人且无异常）→ 优先判定为误听，不报；声源在画面外的要报也必须听感自身清晰确凿
- 人说话归 speeches""",
    # spec_md 引入「画面互证」后不再视觉无关，audio-only 路由必须用本变体：纯音频轮
    # 判定只凭听感本身——若沿用视觉版，「画面视野内看不到迹象」在无画面时恒空真，
    # 模型会据此把真实的玻璃碎/婴儿哭降级为"误听"（安全事件被误抑制）。
    spec_md_audio="""## env_sounds
- 只报「突发、短时、有明确事件含义」的非人声事件（玻璃破碎、警报、犬吠、敲门、婴儿哭、重物倒地…为例）；否则省略该字段
- 持续稳定的运行声 / 嗡鸣 / 底噪一律是背景，不报——空调、抽油烟机、冰箱、各类电器运行、锅具沸腾、键盘敲击、待机音、正常脚步等都属此类
- 微弱、模糊、听不清在响什么 → 不硬识别、不输出（宁缺毋滥）
- 报事件声必须有**具体声学证据**：听感上能说清那是什么样的声音（质地/次数/持续，如「两声清脆的碎裂声」「连续急促的哭声」）才算听到了；只是「有点像 / 大概是」某类事件 → 证据不足，不报。本轮无画面，判定只凭听感本身，不作任何画面推断
- 人说话归 speeches""",
    requires_audio=True,
)
...
- 声音类隐患的门槛（防幻听）：必须听感清晰、能说清听到的是什么声（质地/次数/持续说得出来才算听到了）；并与画面互证——画面空无一人、无任何对应迹象而只有「疑似」声响（疑似玻璃碎 / 疑似婴儿哭 / 疑似呻吟）→ 证据不足，不报。
...
- 报声音类隐患必须有**具体声学证据**：能说清那是什么样的声音（质地/次数/持续，如「两声清脆的碎裂声」「连续急促的哭声」）才算听到了；只是「有点像 / 大概是」某类事件 → 证据不足，不报。本轮无画面，判定只凭听感本身。
