...
#   · 硬约束:句末点名的输出字段名必须与 schema 一致(由 test_note_field_name_matches_schema 守)。
#     解析侧对旧键名留了兼容回退,所以这里写错**不会报错**、只会白增模型写错键的概率 —— 正因为
#     不报错,只能靠那条用例守。
...
def _pick_largest_crop(
    ...
    时刻的框去裁这一帧",人一动就裁到背景或隔壁那个人 —— 而图是绑 track_id 的,绑错等于喂一条
    错的身份证据。所以这里宁可整条放弃 + 告警,也不静默截断:静默截断会让这类错位无任何可观测
    信号,而它的表现不是报错、是图悄悄换成了别人。
    """
