...
class TrackIdentityState:
    ...
    # 装得下的帧数, 要不超过 track 存活上限(当前跟踪器那一份 max_age_sec), 要不超过
    # fast 模式重抽 ReID 的间隔, 才可能出现整窗没有新特征入队的情形。
    # **当前值落在哪一边刻意不写在这里** —— 写进注释就会随调参失真, 这段注释此前正是
    # 这样错的。要确认时按上面两条关系现算, 别信任何写死的结论。
    # 注: 本分支一度加过一个集中校验这类跨模块关系的模块, 已移除 —— 它属于附加项, 而
    # 「配置热更之后由谁重新校验」需要单独设计, 不在本次改动范围内。
