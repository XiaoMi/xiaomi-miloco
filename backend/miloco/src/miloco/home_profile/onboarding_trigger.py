...
[PR #279 fork 改动]
- 新增 ``is_omni_ready`` 条件：配完模型才触发（防 install 中途并发）
- 条件不满足时每 30s 重试（覆盖"先绑账号后配模型"时序），最多 10 分钟
- ``is_omni_ready`` 直接读 config.json 而非 ``get_settings()``（后者有 lru_cache
  不随 ``miloco-cli config set`` 更新）
"""
    ...
    async def maybe_trigger(self) -> bool:
        ...
        for attempt in range(20):  # 最多等 10 分钟
            ...
            try:
                ...
                profile_empty = not self._has_profile_entries()
                omni_ok = self._is_omni_ready()
            except Exception:
                logger.warning("onboarding trigger: 条件检查失败", exc_info=True)
                return False
