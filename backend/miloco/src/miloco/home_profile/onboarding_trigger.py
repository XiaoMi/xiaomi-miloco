...
# 让 agent 转述给用户的部分一律用不指名的第一人称：miloco 是能力来源不是 agent 的身份，
# 插件的 B_IDENTITY 就在同一轮上下文里、明令不得改口自称 Miloco（同 welcome_service）。
# 首句"检测到 miloco 已完成米家授权"说的是系统而非 agent 自称，保持原样。
_INSTRUCTION = (
    "[系统事件] 检测到 miloco 已完成米家授权，但家庭成员与家庭档案均为空——"
    "这应该是一次全新安装，用户还没做过家庭信息初始化。\n"
    "你现在就在用户的聊天会话里，这条回复用户会直接看到。请使用 miloco-onboarding skill "
    "立即向用户开口发起邀请：打招呼、用一两句话说明登记家庭信息的好处（你会更懂这家人，"
