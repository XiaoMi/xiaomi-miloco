...
async def create_rule(rule: Rule, current_user: str = Depends(verify_token)):
    ...
    """
    logger.info("Create rule - User: %s, Name: %s", log_safe(current_user), log_safe(rule.name))
...
async def get_all_rules(
    ...
    """Get all rules"""
    logger.info(
        "Get all rules - User: %s, enabled_only: %s", log_safe(current_user), log_safe(enabled_only)
    )
...
async def get_logs(
    ...
    logger.info(
        "Get rule logs - User: %s, limit: %s, after_ts: %s, before_ts: %s, kind: %s",
...
async def cleanup_logs(
    ...
    logger.info(
        "Cleanup rule logs - User: %s, keep_days: %s",
        log_safe(current_user),
        log_safe(keep_days),
    )
...
async def trigger_rule(
    ...
    """
    logger.info("Trigger rule - User: %s, ID: %s", log_safe(current_user), log_safe(rule_id))
...
async def get_rule(rule_id: str, current_user: str = Depends(verify_token)):
    ...
    logger.info("Get rule - User: %s, ID: %s", log_safe(current_user), log_safe(rule_id))
...
async def update_rule(
    ...
    logger.info("Update rule - User: %s, ID: %s", log_safe(current_user), log_safe(rule_id))
    rule.id = rule_id
...
async def patch_rule(
    ...
    logger.info("Patch rule - User: %s, ID: %s", log_safe(current_user), log_safe(rule_id))
...
async def delete_rule(rule_id: str, current_user: str = Depends(verify_token)):
    """Delete a rule"""
    logger.info("Delete rule - User: %s, ID: %s", log_safe(current_user), log_safe(rule_id))
...
async def get_logs_by_rule(
    ...
    before_ts = parse_iso_ms(before, "before") if before is not None else None
    logger.info(
        "Get rule logs - User: %s, rule_id: %s, limit: %s, after_ts: %s, "
        "before_ts: %s, kind: %s",
        log_safe(current_user),
        log_safe(rule_id),
        log_safe(limit),
