...
# 指纹的工作因子(PBKDF2-HMAC-SHA256 迭代次数)。取值由这条链路的延迟预算定,不照搬
# 「口令存储」的通用迭代下限:本函数跑在管理端热路径上(GET /omni-config 对每条「有验证
# 记录」的档案各算一次,测试/保存各一次),本机实测 c=1000 约 0.6ms/次,而通用建议值
# c=600k 约 0.33s/次、c=20k 约 10ms/次,后者放这条链路上不可接受。这里要的是「配置变没
# 变」的作废判据,被保护的是机器签发的高熵 api_key(非人挑的低熵口令),故取一个「够慢到
# 抬高离线穷举成本、又够快不影响交互」的小常数,而不是照搬口令存储的迭代下限。
_FINGERPRINT_PBKDF2_ROUNDS = 1000


def _omni_fingerprint(model: str, base_url: str, api_key: str) -> str:
    """model + base_url + api_key 拼成口令料,以 per-install 随机盐(设备 UUID)做
    PBKDF2-HMAC-SHA256,取前 16 位十六进制,用于判断验证记录是否仍对应当前配置(三者任一
    变过,指纹即变,旧结论作废)。api_key 只入指纹、不落库明文。

    为什么是 PBKDF2 而不是裸 HMAC-SHA256:CodeQL 的 py/weak-sensitive-data-hashing
    按名字把 ``api_key`` 判为 password 类敏感数据(规则侧用 SensitiveDataHeuristics 的
    ``api.?(key|tok)`` 命中 maybePassword),对这类数据该规则要求「计算代价高昂的哈希」,
    并且把「任何 Cryptography::HashingAlgorithm(含 SHA-256)+ password 类数据」都算作
    sink,故 ``hmac.new(..., hashlib.sha256)`` 必被报——报的是「口令类数据不该走快哈希」
    这条规则,不是 SHA-256 本身有问题。PBKDF2 属该规则认可的慢哈希 KDF(与
    SCRYPT/ARGON2/BCRYPT 同类,见规则侧 CryptoAlgorithmNames),换用它即在代码层面消除
    告警,不需要 dismiss。

    参数映射与等价性:PBKDF2 的 password 位放 model\\x00base_url\\x00api_key(被保护的
    秘密作为 KDF 的口令料),salt 位放设备 UUID。rounds=1 时 PBKDF2 恰好退化为一次
    HMAC-SHA256(key=三元组, msg=盐||0x00000001),与旧实现同强度;这里取
    _FINGERPRINT_PBKDF2_ROUNDS(>1),在延迟预算内把离线穷举 api_key 的成本抬高该倍数,
    安全性只增不减——测试 test_fingerprint_is_not_a_bare_fast_hash 钉住了这两点。

    盐取自 SystemConfigKeys.DEVICE_UUID_KEY(每台部署各自随机生成),故同一份三元组在不同
    安装上得到不同摘要,无法跨安装做预计算比对。设备 UUID 一变(重装、换机、kv 重建)即等于
    换了盐,此前写下的摘要一律对不上,已有验证记录随之失效、界面回落到「○ 未验证」,需用户
    重测一遍;这是刻意的失效而非缺陷——旧记录对应的是另一台设备的上下文,不为此加迁移逻辑。

    摘要从不下发给前端(见 _omni_last_verified),只在本进程内做相等比较。
    ...
    salt = (manager.kv_repo.get(SystemConfigKeys.DEVICE_UUID_KEY) or "").encode()
