...
「凭据可能不好使」——而它同时是感知侧相机发现的闸门，于是一次网络抖动就会让
全部摄像头掉线。把健康度拆成独立的一份状态之后，`_oauth_info` 回归它字面的
含义（凭据在不在），感知的闸门改看 `is_operational`：瞬时故障期间照跑，只有
云端明确拒绝凭据才停——那一档本来就拉不到相机列表，停下是唯一诚实的表现。
