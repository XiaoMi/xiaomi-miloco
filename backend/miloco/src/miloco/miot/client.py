...
class MiotProxy:
    ...
    async def read_cameras_awake(
        ...
        镜头→通道映射由 spec ``service_description`` 决定（含「球机」→ch0 / 含「枪机」→ch1）：
        有球/枪机标签 = 多摄，按标签分路、主开关（无标签，是枪机镜像）忽略；无任何球/枪机标签
        = 单摄，全部开关 OR 后归 ch0（与旧整机 OR 等价）。同一路多开关按 OR 合并（任一 True→
        True；否则任一 None→None；全 False→False，不因个别读失败误判关闭）。
