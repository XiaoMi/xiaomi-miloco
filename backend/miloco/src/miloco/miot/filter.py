...
def _as_day_intervals(start: int, end: int) -> list[tuple[int, int]]:
    ...
    if start < end:
        return [(start, end)]
    # 跨午夜：切成「今天的尾巴 + 次日的开头」。end == 0 表示恰好到午夜收尾，
    # 次日那半是零长，直接不产出——否则会留一个 (0, 0) 退化片给重叠检查。
    intervals = [(start, 24 * 60)]
    if end > 0:
...
def set_camera_schedule(kv_repo: KVRepo, did: str, schedule: dict) -> tuple[dict, bool]:
    ...
    # 关闭定时时不擦用户数据：没显式给的字段沿用库里的值。注意判断「调用方没指定
    # 星期」只能靠 None——CameraSchedule.weekdays 缺省就是 None（default_factory
    # 会让请求里永远是个 list，这个分支曾经因此不可达）。
    if (
