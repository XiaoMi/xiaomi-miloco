...
class LocalVisionSettings(BaseModel):
    ...
    container_fps: int = Field(
        ...
        description=(
            ...
            "(双相机 3.5s→5.5s)。所以默认取 8 这个折中,而不是拉满 —— 想要更细的时间"
            "分辨率就往上调,同时把 period_sec 一并调大"
        ),
    )
    crf: int = Field(
    ...
    codec_target_canvas: int = Field(
        ...
        description=(
            ...
            "家庭常驻感知要留波动余量,取 12。"
            "官方默认 32 需要 256 帧;填 32 而喂不满时模型会自己降到 28 并警告,"
            "所以直接写 28,结果一样但不产生噪声。两者必须一起改。"
            "注意需要 (值/4)×32 帧才能填满,填不满会自动降到实际能填的档位"
        ),
    )
