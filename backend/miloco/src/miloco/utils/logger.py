...
def log_safe(value: object) -> str:
    ...
    import 一个私有名字是更差的选择。
    """
    # 两个字符都换成**空格**而不是删掉：删掉会把「关灯\n晚安」拼成「关灯晚安」，与
    # 一个真就叫那个名字的任务在日志里再也分不开，按名字 grep 会互相串；多行的异常
    # 消息同样会被拼成一串没有分隔的文字。而「不让它伪造出额外整行」这个目的，换成
    # 空格一样达成。CRLF 先整体换一次，免得先换 \r 再换 \n 留下两个空格。
    # 保持 `replace` 首参是**字面量**的形态：CodeQL 只认第一个实参是字面量
    # "\n" / "\r\n" 的 replace 为 sanitizer，改成正则或变量会重新触发
    # py/log-injection 告警。
    return (
        str(value)
        .replace("\r\n", " ")
