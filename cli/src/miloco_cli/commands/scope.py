...
# 查状态分两层看 scope camera list：crop_in_use = 这里设成了什么（**存储偏好**，全局闸
# 关着时它照样是 true）；crop_effective = 三道闸是否全开（= in_use AND 全局双闸 AND
# crop_in_use）。问「这一路到底在不在裁」只能看后者，crop_in_use 答不了。
