...
def _find_latest_log() -> Path | None:
    """查找 backend 日志文件(固定名 miloco-backend.log,轮转的 .1….20)。

    轮转归属按平台不同但布局一致(10MB × 20):macOS(launchd) 由 backend 自管
    (utils/log_rotation.py);Linux(supervisord) 仍由 supervisord 做,backend 侧被
    ``MILOCO_SUPERVISED=1`` 显式关掉 —— 改动这两处任一都会让 Linux 变成「两边都不轮转」。

    历史上 daemon 模式用过 per-boot 时间戳名 ``miloco-backend_<ts>.log``;该形态自
    launchd 迁移起不再产生,仅为从旧版升级上来、目录里还留着旧文件的机器保留 fallback。
    """
    log_dir = _log_dir()
...
def _generate_launchagent_plist(cmd: list[str]) -> None:
    ...
    #
    # 不合并进 miloco-backend.log 是因为**轮转归属**:launchd 无父侧轮转,接管 fd
    # 之前那段没人管,分开才能对它用粗暴裁剪(别拿"偏移会写坏"当理由——实测 launchd
    # 以 O_APPEND 且不截断打开,交错追加是安全的)。
...
def _stdio_log_field() -> dict:
    """``service status`` 附加的 launchd 兜底日志字段(文件不存在则空)。

    加键而非改 ``log_file``:hermes 插件在解析这份 JSON。按存在性判断,Linux 上零变化。
    """
    path = _launchd_stdio_path()
    try:
        if path.exists():
            return {"stdio_log_file": str(path)}
    except OSError:
        pass  # 目录不可读(EACCES)等:当作没有这个文件,status 不该因此失败
    return {}


def _stdio_newer_than_backend(backend: Path, stdio: Path) -> bool:
    """兜底日志是否比主日志更新 —— "后端崩在接管 stdio 之前" 的判据。

    后端健康时主日志一直在写,stdio 更旧;停机时两边都不动。用 mtime 绝对值做窗口
    反而会在停机后恒真。
    """
    try:
        return stdio.stat().st_mtime > backend.stat().st_mtime
    except OSError:
        return False
...
@click.option(
    "--source",
    type=click.Choice(["auto", "backend", "stdio"]),
    default="auto",
    show_default=True,
    help="看哪个日志：auto=主日志优先，缺失时兜底 launchd 的 stdio 日志",
)
def service_logs(follow, lines, source):
    """查看服务日志。

    ``auto`` 下主日志优先,不存在时兜底到 ``launchd-stdio.log`` —— 后端没走到接管
    stdio 那一步时,它是唯一线索。
    """
    log_dir = _log_dir()
    if not log_dir.exists():
    ...
    stdio = _launchd_stdio_path()
    backend = _find_latest_log()

    if source == "stdio":
        if not stdio.exists():
            click.echo(f"no launchd stdio log: {stdio}", err=True)
            sys.exit(1)
        chosen = stdio
    elif source == "backend":
        if backend is None:
        ...
        chosen = backend
        # 只提示、不换文件——静默换文件比给一个陈旧的已知文件更意外。
        if _stdio_newer_than_backend(backend, stdio):
            click.echo(
                f"提示：{stdio.name} 比主日志更新，后端可能崩在接管 stdio 之前；"
                "看那一段：miloco-cli service logs --source stdio",
                err=True,
            )
    elif stdio.exists():
        chosen = stdio
        click.echo(
            "miloco-backend.log 不存在（后端没走到接管 stdio 那一步），"
            f"改看 launchd 兜底日志：{stdio}",
            err=True,
        )
    else:
        click.echo(f"no backend log in {log_dir}", err=True)
    ...
    if follow:
        ...
        # 被替换时重开(BSD/GNU tail 都支持)。兜底日志是原地截断,-F 同样跟得住。
        cmd.append("-F")
