...
def test_logs_follow_uses_capital_F(monkeypatch, tmp_path):
    ...
    service.service_logs.callback(lines=50, follow=True, source="auto")
    assert "-F" in captured["args"] and "-f" not in captured["args"]


def _capture_logs(monkeypatch, log_dir):
    """把 _log_dir 指到 tmp，并拦下 os.execvp 拿 tail 收到的参数。"""
    captured: dict = {}
    monkeypatch.setattr(service, "_log_dir", lambda: log_dir)
    monkeypatch.setattr(
        service.os, "execvp", lambda file, args: captured.update(file=file, args=args)
...
def test_logs_falls_back_to_launchd_stdio(monkeypatch, tmp_path, capsys):
    """主日志不存在时兜底到 launchd-stdio.log，并说明换了文件。

    原先只回一句 "no backend log" 再 exit 1，用户连该去哪看都不知道。
    """
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    stdio = log_dir / "launchd-stdio.log"
    stdio.write_text("Traceback (most recent call last):\n")
    captured = _capture_logs(monkeypatch, log_dir)

    service.service_logs.callback(lines=50, follow=False, source="auto")

    assert captured["args"] == ["tail", "-n50", str(stdio)]
    assert "launchd-stdio.log" in capsys.readouterr().err


def test_logs_keeps_backend_log_when_stdio_is_newer(monkeypatch, tmp_path, capsys):
    """兜底日志更新时只提示、不换文件（提示里已带 `--source stdio`，选择权留用户）。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    backend = log_dir / "miloco-backend.log"
    backend.write_text("main log\n")
    (log_dir / "launchd-stdio.log").write_text("Traceback\n")
    old = time.time() - 600
    os.utime(backend, (old, old))
    captured = _capture_logs(monkeypatch, log_dir)

    service.service_logs.callback(lines=50, follow=False, source="auto")

    assert captured["args"] == ["tail", "-n50", str(backend)]
    assert "--source stdio" in capsys.readouterr().err


def test_logs_no_hint_when_backend_log_is_fresh(monkeypatch, tmp_path, capsys):
    """正常态不能挂提示：主日志一直在写，stdio 恒更旧（用绝对时间窗会误报）。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    backend = log_dir / "miloco-backend.log"
    backend.write_text("main log\n")
    stdio = log_dir / "launchd-stdio.log"
    stdio.write_text("objc[4799]: Class AVFFrameReceiver is implemented in both ...\n")
    old = time.time() - 600
    os.utime(stdio, (old, old))
    captured = _capture_logs(monkeypatch, log_dir)

    service.service_logs.callback(lines=3, follow=False, source="auto")

    assert captured["args"] == ["tail", "-n3", str(backend)]
    assert capsys.readouterr().err == ""


def test_logs_source_stdio_forces_stdio(monkeypatch, tmp_path, capsys):
    """--source stdio 无视主日志直接给兜底日志（提示里指的就是这条命令）。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    (log_dir / "miloco-backend.log").write_text("main log\n")
    stdio = log_dir / "launchd-stdio.log"
    stdio.write_text("objc[4799]: ...\n")
    captured = _capture_logs(monkeypatch, log_dir)

    service.service_logs.callback(lines=50, follow=False, source="stdio")

    assert captured["args"][-1] == str(stdio)
    assert capsys.readouterr().err == ""


@pytest.mark.parametrize(
    ("source", "expected"),
    [("backend", "no backend log"), ("stdio", "no launchd stdio log")],
)
def test_logs_explicit_source_missing_exits(
    monkeypatch, tmp_path, capsys, source, expected
):
    """显式指定却拿不到时必须硬失败：静默回落到另一个文件会让 --source 变成谎言。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    _capture_logs(monkeypatch, log_dir)

    with pytest.raises(SystemExit) as exc:
        service.service_logs.callback(lines=50, follow=False, source=source)

    assert exc.value.code == 1
    assert expected in capsys.readouterr().err


def test_logs_auto_exits_when_no_log_at_all(monkeypatch, tmp_path, capsys):
    """两个都不存在时保持原有行为（exit 1），别把 auto 变成"永远成功"。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    _capture_logs(monkeypatch, log_dir)
    ...
    assert exc.value.code == 1
    assert "no backend log" in capsys.readouterr().err


def test_launchd_status_includes_stdio_log_file(monkeypatch, tmp_path):
    """钉住 helper 真的接到了 status 上（只测 helper 会漏掉接线），且 log_file 语义未变。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    stdio = log_dir / "launchd-stdio.log"
    stdio.write_text("objc[4799]: ...\n")
    ...
    monkeypatch.setattr(service, "_launchd_backend_pid", lambda: 4321)
    monkeypatch.setattr(service, "_process_uptime_seconds", lambda pid: 7)
    printed: list = []
    monkeypatch.setattr(service, "print_result", lambda data, pretty: printed.append(data))

    service._launchd_status({"server": {"url": "http://127.0.0.1:1810"}}, pretty=False)

    assert printed[0]["stdio_log_file"] == str(stdio)
    assert printed[0]["log_file"] is None


def test_stdio_log_field_absent_when_file_missing(monkeypatch, tmp_path):
    """文件不存在时整个键缺席（Linux 上本来就没有这个文件，行为零变化）。"""
    log_dir = tmp_path / "log"
    log_dir.mkdir()
    monkeypatch.setattr(service, "_log_dir", lambda: log_dir)

    assert service._stdio_log_field() == {}


def test_stdio_log_field_swallows_unreadable_dir(monkeypatch):
    """目录不可读（EACCES）时 status 不该整个失败 —— 少一个诊断字段而已。"""

    class _Unreadable:
        def exists(self):
            raise PermissionError(13, "Permission denied")

    monkeypatch.setattr(service, "_launchd_stdio_path", lambda: _Unreadable())
