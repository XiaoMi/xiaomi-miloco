...
| 扩展类型     | 数量 | 职责概述                                                                                                     |
| ------------ | ---- | ------------------------------------------------------------------------------------------------------------ |
| **配置**     | —    | `$MILOCO_HOME` 路径解析（基于 `get_hermes_home()` 派生）、共享 config.json 读写                              |
...
| **Tools**    | 3    | `miloco_im_push`、`miloco_notify_bind`、`miloco_habit_suggest`——与 OpenClaw 同名同语义                       |
...
三端（backend / CLI / plugin）共用 `$MILOCO_HOME/config.json`，schema 与 OpenClaw 完全一致，差异仅在默认值（webhook_url 指向 bridge 的 `:18789`、auth_bearer 默认空）。插件级私有配置（`debug`、`omni` 模型参数、`notify_session_key`、`bridge_host` / `bridge_port` / `bridge_auth_token` 等）从 Hermes config.yaml 的 `plugins.entries.miloco` 段读取，合并默认值，字段完整列表见 `plugins/hermes/config.py`。
...
| skills 复制时机     | `prebuild`（构建前，`sync-skills.mjs`）        | `pre-commit`（提交前，Python 脚本，跨平台）                    |
| skills 是否进仓库   | 否（`.gitignore` 排除，发布到 npm 包）        | **是**（`git clone` 需要文件存在）                             |
| 访问命名空间       | `<skill-name>`（无命名空间）                  | `miloco:<skill-name>`（插件命名空间，只读，不在系统提示索引）  |

具体复制脚本与阈值不写，分别指向 `plugins/openclaw/scripts/sync-skills.mjs` 与 `plugins/hermes/` 下的 pre-commit 脚本。
