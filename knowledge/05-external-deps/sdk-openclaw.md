...
| **Tools**        | 4    | `miloco_im_push`（通知分发）、`miloco_notify_bind`（通知渠道绑定）、`miloco_notify_unbind`（通知渠道解绑）、`miloco_habit_suggest`（习惯建议状态） |
...
- 插件级私有配置（`debug`、`omni` 模型参数、`notifySessionKeys` 等）存储在 `config.json::plugins.entries[miloco].config` 段；`notifySessionKey` 仅保留兼容旧单通道配置，读取时会自动并入新列表；字段完整列表见 `plugins/openclaw/src/` 各模块注释
