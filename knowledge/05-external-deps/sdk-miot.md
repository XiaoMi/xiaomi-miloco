...
- **MQTT**：通过 `MIoTMipsCloud`（`mips_cloud.py`）订阅五类推送事件——用户设备绑定 / 解绑、设备 meta 变更（含跨家庭移入）、家庭场景变更（改 / 删 / 重命名）、设备云端上线 / 离线状态、设备属性变化。绑定与移入驱动设备欢迎，场景变更驱动场景列表刷新，上线 / 离线事件驱动缓存的摄像头在线状态刷新（事件化替代轮询），上下线与属性变化两类还向 Miloco 侧扇出、写进运行时状态容器（见 [设备控制 · 设备状态容器](../03-features/device-control.md#设备状态容器)）。订阅逐个记账（设备 meta / 上下线 / 属性按设备，场景按家庭），断线重连时整批重放——推送只在值变化时来，漏订阅期间的变化不会补发
