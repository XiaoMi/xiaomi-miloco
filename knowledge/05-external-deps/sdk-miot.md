...
- **本地中枢控制（MIPS）**：与中枢网关建 mTLS MQTT（:8883）后走 MIPS TLV RPC 做属性读写 / 动作调用，是设备控制的首选通道，云端 HTTP 退居兜底（`central_hub.py` / `mips_local.py` / `cert.py`；属性 / 事件订阅接口在 `mips_local.py` 内已实现，尚未向上暴露接线，本地推送留待后续）
- **MQTT**：通过 `MIoTMipsCloud`（`mips_cloud.py`）订阅五类推送事件——用户设备绑定 / 解绑、设备 meta 变更（含跨家庭移入）、家庭场景变更（改 / 删 / 重命名）、设备云端上线 / 离线状态、设备属性变更（`device/{did}/up/properties_changed/#`）。绑定与移入驱动设备欢迎，场景变更驱动场景列表刷新，上线 / 离线事件驱动缓存的摄像头在线状态刷新（事件化替代轮询），与米家保持同步
