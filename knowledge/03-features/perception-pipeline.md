...
感知后端二选一，在「模型」页切换（默认云端，不主动切换则行为不变）：

|          | 云端 API（默认）      | 本地视觉                                                                   |
| -------- | --------------------- | -------------------------------------------------------------------------- |
| 凭据     | 需多模态模型 API Key  | 无需任何 Key                                                               |
| 模态     | 视觉 + 音频           | 仅视觉                                                                     |
| 身份识别 | 有（Omni 调用内完成） | 有，但不经大模型：本地 ReID 比对身份库得出名册，模型只按 bbox 贴名字       |
| 设备执行 | STATIC 规则可直连设备 | 不执行；带直连动作的规则会被拒绝切换，已在本地通路时动作被拒并记为失败触发 |
| 硬件     | 无要求                | 需 GPU（推理跑在独立边车服务里）                                           |

本地通路的取舍是刻意的：没有音频输入就不产 `speeches` / `env_sounds`（让看不见音频的模型
填这两个字段只会得到凭画面脑补的结果，与 `requires_audio` 的门控同一条理由）；不直连设备
则把"做什么"的判断统一收敛到 Agent。

设备动作这条边界是**执行时**强制的，不是切换时声明一下就完事：切换到本地通路时，若有
启用中的直连设备规则，切换会被 400 拒绝并列出它们；而切换之后再新建 / 改出这类规则（或
任务激活时批量启用、直接改 config.json）同样进不去执行——`RuleRunner._fire` 在选到静态
槽位时会查 `perception_executes_device_actions()`，拒绝并记一条 `RULE_TRIGGER_FAILURE`。
两道门缺一不可：前者是转移检查，后者才是状态不变量。

这条保证的边界要说清楚:它约束的是**感知层**不自己直连设备。DYNAMIC 规则命中后
仍会照常投递给 Agent,而 Agent 是可以驱动设备的 —— 也就是说风险是被"先经一次
LLM 判断"所**缓解**,不是被消除。选择这条边界的理由是:纯视觉模型没有音频佐证,
身份也只来自本地 ReID 的相似度比对(有已知失效模式:阈值从未拿人标定、库过期会高
置信度认错),不该由它直接触发预设动作;经 Agent 则至少多一层上下文与判断。

刻意**不做**运行时把 STATIC 改写成 DYNAMIC：改写会丢掉 `cooldown_minutes` / `idempotent`
（schema 里唯一的限流手段）和行动台账里 `source=rule` 的归属，还要把用户配好的动作重新
翻译成一句自然语言交给 Agent 重新理解。
