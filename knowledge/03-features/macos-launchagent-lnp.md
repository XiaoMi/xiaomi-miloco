...
| 日志       | `stdout_logfile` + `redirect_stderr` → `miloco-backend.log`          | `StandardOutPath/StandardErrorPath` → `launchd-stdio.log`（仅兜底 backend 接管 fd **之前**的输出，`service start/stop/restart` 时超 1MB 即裁到最后 200 行）；主日志 `miloco-backend.log` 由 backend 自写并按 10MB × 20 自转 |
