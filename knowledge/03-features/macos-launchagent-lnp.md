...
**运行时唯一替换点**：`cli/src/miloco_cli/commands/service.py`。命令入口按平台选路——`_use_launchd() = (sys.platform=='darwin')` 为真则走 launchd 实现，否则走原 supervisord 实现。**运行时分叉只有这一处**：openclaw / hermes 插件、install.py、人工命令**全部经 `miloco-cli service`**，所以它们在 mac 上自动、透明地用上 launchd，调用方无感。部署链上另有 install.py / sync-to-remote.sh / install-hermes.sh 三处平台或存在性分支，见下文。
...
| 日志       | `stdout_logfile` + `redirect_stderr` → `miloco-backend.log`          | `StandardOutPath/StandardErrorPath` → `launchd-stdio.log`（仅兜底 backend 接管 fd **之前**的输出，`service start/stop/restart` 时超 1MB 即裁到最后 200 行）；主日志 `miloco-backend.log` 由 backend 自写并按 10MB × 20 自转。`service logs` 在主日志缺失时回退到它（`--source stdio` 可指定） |
...
- launchd 无 supervisord 的 unix socket 控制通道；`startretries=3→FATAL` 换成 `KeepAlive={SuccessfulExit:false}` + launchd ~10s 节流。launchd 本身无"放弃"态,但 CLI 侧 `service start/restart` 会在健康探测窗口内检测 crashloop(窗口内见到 ≥3 个不同 pid,即崩溃重生 ≥2 次;单次重启的 2 个 pid 容忍,不误判慢启动)并 `bootout` 停掉、报失败,补上"放弃"语义。
- `stop`（bootout）后 plist 仍留在 `~/Library/LaunchAgents`，**下次登录会因 `RunAtLoad` 再被拉起**（supervisord 的 stop 则一直停到手动再起）。对"应常驻的服务"这是期望行为。
- 日志的**轮转归属**不同：supervisord 由父进程轮转子进程的 stdout（连"子进程没起来"那段也覆盖）；launchd 无父侧轮转，只能由 backend 接管 fd 后自转，接管**之前**那段无人轮转，故单独落在 `launchd-stdio.log` 供 CLI 粗暴裁剪。这也是两文件**不能合并**的真正理由——合并后失控内容会混进可达 10MB 的主日志，无从区分。（实测 launchd 以 `O_APPEND` 且不截断打开，所以"合并会写坏偏移"不成立，别当理由。）
...
- **交叉编译**：Linux 构建端只 `tar` 已签名的 vendored app（不编译）；app 需在 mac 上编译签名一次后入库（步骤见 `launcher/src/miloco_launcher.c` 的 Build 段）。CI 的 `.ci/launcher-guard.py` 会硬校验入库产物仍是双架构 fat 且 Info.plist 身份键未变，并在 `launcher/src/` 改动却未重新入库时告警。
