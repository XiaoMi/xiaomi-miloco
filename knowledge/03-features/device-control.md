...
- **实时事件订阅**：注册 MIPS 云 MQTT 回调（设备改名 / 换房换家、家庭场景变更、设备云端上线 / 离线、设备属性变化），改名 / 换房换家、场景变更、上下线三类经 `mips_listeners.py` 的防抖监听器刷新对应缓存，使本地缓存无需轮询即与云端收敛；四类订阅集各自对账到"当前该订哪些"的目标集，共用一个在飞并发闸，断线重连时整批重放。上下线与属性变化两类还向本层的消费方扇出，写进设备状态容器（见下「设备状态容器」）——属性值本层不缓存，只扇出；设备绑定 / 移入受管家庭触发的设备欢迎见 [device-welcome.md](device-welcome.md)
...
设备的实时状态（每台的在线标志与各属性值）除了随请求现拉，还常驻一份在进程内的分层 KV 状态容器 `StateStore`（`state/store.py`，容器本身的通用语义见 [系统架构总览 · 横切关注点](../01-overview/overview.md#横切关注点)）。设备控制域是它当前唯一的写入方，写在 `iot/` 子树下：一台设备一个节点，在线标志与属性各占一层子树（在线标志不直接挂在设备那一层——那一层全是子树，混一片叶子进去会让少写通配符的快照从报错退化成静默返回残缺结果）。

**三条写入通道，每一笔都记来源**（`dump` 里一眼能看出这条叶子是拉来的还是推来的；补拉与对齐共用同一个来源标记——对补拉来说它就是一次迟到的对齐）：

- **启动对齐**（`miot/state_align.py`）：推送只在值变化时才来，重启后容器是空的、不会自己长回来，所以启动时按当前启用家庭主动拉一遍。离线设备只写在线标志、不拉属性——云端给的是缓存里的最后一次上报、可能任意旧，写进去会把上报时刻刷成当下而消费方看不出来；整台跳过也不行，那样消费方分不出"离线"和"没接入"。对齐失败会退避重试，整代都失败则这一代停在未对齐。
- **推送**（`miot/state_push.py`）：MiOT 的上下线推送与属性变化推送，是对齐之后容器跟上真实世界的唯一途径。
- **上线补拉**（`Manager`）：设备转上线时把它在容器里缺的属性补齐，值仍交给推送写入器落盘；按作用域代限次，用完就停。

**两道闸只有一份，且都在写入的那一刻判**（`IotPushWriter`）：一道判"本代已对齐"，挡住换账号 / 换家庭时旧连接的在途推送写进刚清空的树；一道判"还在当前家庭"，挡住同一代之内搬出当前家庭的设备被在途消息写回来变成删不掉的幽灵。补拉要打一趟云端，往返期间设备可能搬走，所以只在往返前判一次挡不住——补拉侧那几处早退是为省掉无谓的云端往返，不承担正确性。上下线推送只过第二道闸：切账号时启用集先被清空，而家庭过滤对空启用集一律返回假，那个窗口本就是失败关闭的。

**作用域代号**：账号 / 家庭切换递增一个进程内代号（`Manager.begin_scope_switch`），切换后旧作用域的写入一律作废——对齐要打好几秒云端请求，这期间用户可能已经切走，不比代号就会把旧家庭的值写进刚重建的树，而且带的是当下的时间戳，事后看不出它是旧的。真正改变启用集的切换（含账号解绑）走统一入口清空容器、重建缓存后再起新一轮对齐，空切不清；设备列表刷新时不再属于当前家庭的设备由调和通道删除。
