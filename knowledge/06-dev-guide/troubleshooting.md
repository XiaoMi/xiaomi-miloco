...
| 引擎不可用（节点处于 `PREREQ_MISSING`）   | 看响应体 `engine.status` 字段：`no_omni_api_key` → `config set model.omni.api_key`；`models_missing` → 源码开发用 `python3 scripts/fetch_models.py --dest ~/.openclaw/miloco/models` 按 `scripts/models.lock.json` 补齐全部 5 个模型（2 必需 + 3 可选，约 78MB；只想最快解开 `models_missing` 可加 `--required-only` 少下约 25MB，代价是语义去重与 VAD 继续降级）。注意这条补的是**运行时**目录，与 `local-ci.sh` 校验的仓库内包目录是两个地方：它那句「感知模型未按 lock 就绪」要另跑 `python3 scripts/fetch_models.py --strict --dest backend/miloco/src/miloco/perception/models` 才会消失，加不加 `--required-only` 都一样。装过安装包的重跑 `install.sh` 也行；`engine_init_failed` → 看 `miloco-backend.log` |
