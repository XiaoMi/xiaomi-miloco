...
| 9000      | 系统错误          | 未捕获的内部异常；CLI 或原始 API 响应中的 `data.error_id` 与后端日志中的 `error_id=` 一一对应，排障时先记录该 ID，再执行 `grep 'error_id=<id>' $MILOCO_HOME/log/miloco-backend.log`。Web 面板当前只显示固定错误消息，不显示该 ID |
