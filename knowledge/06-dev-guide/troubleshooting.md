...
| 引擎不可用（节点处于 `PREREQ_MISSING`）   | 看响应体 `engine.status` 字段：`no_omni_api_key` → `config set model.omni.api_key`；`models_missing` → 源码开发用 `python3 scripts/fetch_models.py --dest ~/.openclaw/miloco/models` 按 `scripts/models.lock.json` 补齐 2 个必需 ONNX（装过安装包的重跑 `install.sh` 也行）；`engine_init_failed` → 看 `miloco-backend.log` |
