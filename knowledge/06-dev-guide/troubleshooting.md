...
| 日志              | 路径                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Server            | `$MILOCO_HOME/log/miloco-backend.log`（实时：`miloco-cli service logs -f`；不存在时自动兜底到 macOS 的 `launchd-stdio.log`，或 `--source {backend,stdio}`）                                                                                                                                                                                                                                  |
| 进程托管（Linux） | `$MILOCO_HOME/log/supervisord.log`                                                                                                                                                                                                                                                                                                                                                           |
| 进程托管（macOS） | `$MILOCO_HOME/log/launchd-stdio.log`（launchd 只兜底 backend 接管 stdio 之前的输出；`service start/stop/restart` 时若超 1MB 会裁到最后 200 行——后端在接管 fd 前崩溃循环时它会持续累积；`service logs` 在主日志缺失时回退到它（`--source stdio` 可指定），`service status` 的 `stdio_log_file` 同指；权限问题见 [macOS LaunchAgent 与本地网络权限](../03-features/macos-launchagent-lnp.md)） |
