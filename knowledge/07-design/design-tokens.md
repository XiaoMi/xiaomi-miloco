...
图表里要同屏区分的**几路并列分类**不借用语义色,单开一族 `--color-usage-*`(四路分类色 + 一个中性单序列色,色值见 `theme.css`)。

| token                                                         | 用途                                                    |
| ------------------------------------------------------------- | ------------------------------------------------------- |
| `usage-text` / `usage-video` / `usage-audio` / `usage-output` | 用量页模态构成的四路分类(环形图 / 堆叠柱)               |
| `usage-total`                                                 | 「不分模态」的中性单序列,刻意不复用上面四色中的任何一个 |

为什么不借现成的色:

- **语义色是状态编码**,借来当序列色会让状态色失去保留含义;且品牌橙与警告橙在模拟红绿色盲下几乎分不开,同屏不可用。
- **品牌可视化调色板是单序列橙色阶梯**(见 [小米品牌语言 · 数据可视化调色板](xiaomi-brand-language.md#25-数据可视化调色板)),为层次而非分类设计,撑不住四路。

判据与约束:

- 四色按**链式相邻**与**环形首尾回绕**两种配对方式都要过「色盲模拟色差不低于 8、正常视力不低于 15」。
- 深色档按暗底亮度带**重新取值**,不是把浅色档反转;`theme.css` 里显式浅色、显式深色、跟随系统的深色分支**三处都要补**(第三处最易漏)。
- 浅底上有两色对非文字元素达不到 3:1,靠**图例直接标出名称与数值**兜底 —— 所以这类图的图例不能被精简成只有色块。
- `usage-` 前缀是有意的:暂不对全站图表做承诺,别把它当通用图表色族外扩。
